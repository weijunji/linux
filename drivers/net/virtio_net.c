...
#include "../infiniband/hw/virtio/virtio_rdma.h"
#include "../infiniband/hw/virtio/virtio_rdma_ib.h"
#include "../infiniband/hw/virtio/virtio_rdma_queue.h"
...
static int virtnet_find_vqs(struct virtnet_info *vi)
{
	...
	if (virtio_has_feature(vi->vdev, VIRTIO_NET_F_ROCE)) {
		total_vqs += 1 + vi->max_cq + vi->max_qp * 2;
	}
	...
	/************** virtio-rdma ****************/
	names[net_vqs] = "rdma.ctrl";
	callbacks[net_vqs] = NULL;
	...
	for (i = 0; i < vi->max_cq; i++, cur_vq++) {
		...
		callbacks[cur_vq] = virtio_rdma_cq_ack;
	}
...
static int virtnet_init_roce(struct virtnet_info *vi) {
	...
	virtio_cread_le(vi->vdev, struct virtio_net_config, max_qp, &vi->max_qp);
	virtio_cread_le(vi->vdev, struct virtio_net_config, max_cq, &vi->max_cq);
	pr_info("qp & cq: %u %u\n", vi->max_qp, vi->max_cq);
	return 0;
}
